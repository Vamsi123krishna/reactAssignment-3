...
class App extends Component {
  state = {
    ...
    statethumbnailUrl: imagesList[0].thumbnailUrl,
    time: 60,
    gameover: false,
  }

  componentDidMount() {
    this.timerId = setInterval(this.timeChanging, 1000)
  }
  ...
  onClickImage = thumbnailUrlValue => {
    const {statethumbnailUrl} = this.state
    const thumbnailValue = imagesList.filter(
      eachItem => eachItem.thumbnailUrl === thumbnailUrlValue,
    )
    const {thumbnailUrl} = thumbnailValue[0]
    if (statethumbnailUrl === thumbnailUrl) {
      const newThumbnailUrl =
        imagesList[Math.floor(Math.random() * imagesList.length)].thumbnailUrl
      this.setState(prevState => ({
        score: prevState.score + 1,
        statethumbnailUrl: newThumbnailUrl,
      }))
    } else {
      clearInterval(this.timerId)
      this.setState({
  ...
  renderGameoverCard = () => {
    ...
    return (
      <div>
        ...
        <button onClick={this.onClickPlayAgain}>
  ...
  renderPlayCard = () => {
    const {score, activeTabId, time, statethumbnailUrl} = this.state
    const filterItemByTab = imagesList.filter(
        ...
        <div>
          ...
          <ul>
            {tabsList.map(eachItem => (
              <TabsItem
                key={eachItem.tabId}
                eachItemTab={eachItem}
                updateTabItem={this.updateTabItem}
              />
            ))}
          </ul>
          <ul>
            {filterItemByTab.map(eachItem => (
              <li key={eachItem.id}>
                <img
                  src={eachItem.imageUrl}
                  onClick={this.onClickImage(eachItem.thumbnailUrl)}
                />
              </li>
            ))}
          </ul>
  ...
  render() {
    ...
    return <>{gameover ? this.renderGameoverCard() : this.renderPlayCard()}</>
  }
}
